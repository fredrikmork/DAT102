...
public class LinkedBST<T extends Comparable<T>> implements BSTreADT<T>, Iterable<T> {
	...
	//Finner høyden på treet
	public int height() {
		return finnHoyde(rot);
	}
	private int finnHoyde(BinaerTreNode<T> p) {
		if (p == null) { //hvis vi har et tomt tre, er høyden -1
			return -1;
		}
		int venstre = finnHoyde(p.getVenstre());
		int hoyre = finnHoyde(p.getHoyre());
